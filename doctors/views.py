...
from patients.models import Time
...
def view_appointments(request):
    ...
    if filter_patient_name:
        appointments = appointments.filter(patient__user__first_name__icontains=filter_patient_name)
...
@login_required(login_url='/login')
def book_appointment_doctor(request):
    ...
    if request.method == 'POST':
        ...
        if not (patient_name and date and time_val and summary):
            messages.error(request, 'Please fill in all required fields.')
        else:
            try:
                # Try to find patient by username or first/last name
                User = get_user_model()
                user_qs = User.objects.filter(username=patient_name)
                if not user_qs.exists():
                    user_qs = User.objects.filter(first_name__iexact=patient_name)
                if not user_qs.exists():
                    user_qs = User.objects.filter(last_name__iexact=patient_name)
                if not user_qs.exists():
                    messages.error(request, 'Patient not found. Please enter a valid patient username or name.')
                    return render(request, 'doctors/book_appointment.html', {'times': times})
                user = user_qs.first()
                patient = Patients.objects.get(user=user)
                time_obj = Time.objects.get(time=time_val)
